...
def store_data(lol_match_data, game_stats, account_id_list, summoner_id, match_id_json, region):
    ...
    with open(game_data_file, 'w') as file_name:
        json.dump(game_stats, file_name)

    with open(used_account_file, 'w') as file_name:
        json.dump(account_id_list, file_name)

    with open(unused_account_file, 'w') as file_name:
        json.dump(summoner_id, file_name)

    with open(matchid_file, 'w') as file_name:
...
def get_account_id_list_v2(riot_key, region_idx):
    ...
    region, summoner = region_selector(region_idx)
    ...
    match_list_api    = 'https://{0}.api.riotgames.com/lol/match/v3/matchlists/by-account/'.format(
        region)
    match_data_api    = 'https://{0}.api.riotgames.com/lol/match/v3/matches/'.format(
        region)
    summoner_info_api = 'https://{0}.api.riotgames.com/lol/summoner/v3/summoners/by-name/'.format(
        region)
    ...
    if len(file_name) >= 1:
        ...
        print('{0} Pulling Data From: {1} With Summoner ID: {2}...'.format(
            ...
            region,
            summoner))
        with open(data_file, "w") as f_name:
    ...
    if len(file_name) >= 1:
        ...
        summoner_id = [summoner_info['accountId']]
    ...
    while len(summoner_id) >= 1:
        ...
        if summoner_id_new not in account_id_list:
            ...
            url = "{0}{1}{2}".format(match_list_api, summoner_id_new, api_key)
            ...
            if account_info is not None and account_info.status_code == 200:
                ...
                matches = account_info['matches']
                game_id_list = [str(v['gameId']) for v in matches]
                ...
                for match_id_new in game_id_list:
                    ...
                    if match_id_new not in match_id_json:
                        ...
                        if match_data is not None and match_data.status_code == 200:
                            ...
                            game_info = [int(match_json['mapId'])] + [int(match_json['queueId'])]
                            game_info += [str(match_json['gameMode'])] + [int(match_id_new)]

                            for match in match_json['participants']:
                                if len(match_json['participants']) == 10:
                                    try:
                                        temp_stats.append(match['championId'])
                                    except KeyError:
                                        temp_stats.append('NA')
                                    try:
                                        temp_stats.append(match['highestAchievedSeasonTier'])
                                    except KeyError:
                                        temp_stats.append('NA')
                                    try:
                                        temp_stats.append(match['spell1Id'])
                                    except KeyError:
                                        temp_stats.append('NA')
                                    try:
                                        temp_stats.append(match['spell2Id'])
                                    except KeyError:
                                        temp_stats.append('NA')
                                    try:
                                        temp_stats.append(match['timeline']['lane'])
                                    except KeyError:
                                        temp_stats.append('NA')
                                    try:
                                        temp_stats.append(match['timeline']['role'])
                                    except KeyError:
                                        temp_stats.append('NA')
                                    temp_stats = temp_stats + game_info

                                    if isinstance(match['stats']['win'], bool) is True:
                                        if match['stats']['win']:
                                            temp_stats.append(1)
                                        else:
                                            temp_stats.append(0)
                                    else:
                                        temp_stats.append(match['stats']['win'])
                                game_stats.append(temp_stats)
                                temp_stats = []

                            # Getting account ids for all the players in the match.
                            try:
                                match_data_player = match_json['participantIdentities']
                                temp_id = [v['player']['accountId'] for v in match_data_player]
                                summoner_id += temp_id
                                temp_id = []
                            except KeyError:
                                pass
